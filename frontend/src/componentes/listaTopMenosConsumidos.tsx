...
export default class ListaTopMenosConsumidos extends Component<props, state> {
                ...
                <div>
                    {
                        topMenosConsumidos.map((item, index) => (
                            <a className={`collection-item avatar black-text truncate ${item[0] === this.state.selectedItem ? estilo : ''}`} key={index} onClick={() => this.handleItemClick(item)}>
